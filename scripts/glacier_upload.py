...
import subprocess
import hashlib
#xfrom treehash import TreeHash
...
for item in file_list:
    ...
    command = """aws glacier upload-multipart-part --upload-id {0} --body {1} --range 'bytes {2}-{3}/*' --account-id - \
               --vault-name {4}""".format(uploadId, item, start_bytes, end_bytes-1, s3_glacier_vault_name)
    ...
    #command = f"openssl dgst -sha256 {zip_file_part} >> {zip_file_hash_name}"
    #result = subprocess.run(command, shell=True)
    #if 0 != result.returncode:
    #   print(f"[ERROR]: Failed to run: `{command}`")
    #   os.sys.exit(-1)

    #os.remove(zip_file_part)

#command = f"openssl dgst -sha256 {zip_file_hash_name}"
#result = subprocess.run(command, shell=True, stdout=subprocess.PIPE)
#if 0 != result.returncode:
#    print(f"[ERROR]: Failed to run: `{command}`")
#    os.sys.exit(-1)

#os.remove(zip_file_hash_name)
#zip_file_hash = result.stdout.rstrip()
